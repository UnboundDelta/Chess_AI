...
#probably better try to place rooks on open files, or on same file as other rook/queens
...
def findMoveNegaMax(gs, validMoves, depth, turnMultiplier):
    ...
    for move in validMoves:
        gs.makeMove(move)
        nextMoves = gs.getValidMoves()
        score = -findMoveMinMax(gs, nextMoves, depth-1, -turnMultiplier)
        if score > maxScore:
            maxScore = score
            if depth == DEPTH:
                nextMove = move
